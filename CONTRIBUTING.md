...
By contributing to VampireLib you agree with the [Developer Certificate of Origin (DCO)][DCO].
...
*This is only a summary of VampireLib's PR process and an explanation of VampireLib-specific exceptions to it. For exact definitions and more information, see [RFC 39](https://github.com/QuiltMC/rfcs/blob/master/structure/0039-pr-policy.md).*
...
If there are two acceptable spellings of the same word, first check if one is already being used in VampireLib, Quilt Mappings
...
present in VampireLib, in other Quilt Mappings names, in libraries used by Minecraft, or in vanilla strings. The rest of this
...
`@Unique` fields must be prefixed with `vl$`, but `@Unique` methods don't need prefixes.
...
VampireLib is licensed under [Apache 2.0][LICENSE], and have a [Developer Certificate of Origin (DCO)][DCO]
...
Every Java source file in VampireLib has a license header with a copyright notice that keeps track of its creation date and
