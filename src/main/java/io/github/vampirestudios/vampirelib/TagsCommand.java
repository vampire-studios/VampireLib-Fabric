...
/**
 * The {@code /vampirelib tags} command for listing a registry's tags, getting the elements of tags, and querying the tags of a
 * registry object.
 *
 ...
 * <ul>
 *     <li>{@code /vampirelib tags &lt;registry> list [page]} - Lists all available tags in the given registry.</li>
 *     <li>{@code /vampirelib tags &lt;registry> get &lt;tag> [page]} - Gets all elements of the given tag in the given registry.</li>
 *     <li>{@code /vampirelib tags &lt;registry> query &lt;element> [page]} - Queries for all tags in the given registry which
...
class TagsCommand {
    ...
    private static final DynamicCommandExceptionType UNKNOWN_REGISTRY = new DynamicCommandExceptionType(key ->
        Component.translatable("commands.vampirelib.tags.error.unknown_registry", key));
    private static final Dynamic2CommandExceptionType UNKNOWN_TAG = new Dynamic2CommandExceptionType((tag, registry) ->
        Component.translatable("commands.vampirelib.tags.error.unknown_tag", tag, registry));
    private static final Dynamic2CommandExceptionType UNKNOWN_ELEMENT = new Dynamic2CommandExceptionType((tag, registry) ->
        Component.translatable("commands.vampirelib.tags.error.unknown_element", tag, registry));

    public static ArgumentBuilder<CommandSourceStack, ?> register() {
        /*
         * /vampirelib tags <registry> list [page]
         * /vampirelib tags <registry> get <tag> [page]
         * /vampirelib tags <registry> query <element> [page]
         */
    ...
    private static int listTags(final CommandContext<CommandSourceStack> ctx, final int page)
        ...
        ctx.getSource().sendSuccess(createMessage(
            Component.translatable("commands.vampirelib.tags.registry_key", Component.literal(registryKey.location().toString()).withStyle(ChatFormatting.GOLD)),
            "commands.vampirelib.tags.tag_count",
            "commands.vampirelib.tags.copy_tag_names",
            tagCount,
            page,
    ...
    private static int listTagElements(final CommandContext<CommandSourceStack> ctx, final int page)
        throws CommandSyntaxException {
        final ResourceKey<? extends Registry<?>> registryKey = getResourceKey(ctx, "registry", ROOT_REGISTRY_KEY)
            .orElseThrow(); // Expect to be always retrieve a resource key for the root registry (registry key)
        final Registry<?> registry = ctx.getSource().getServer().registryAccess().registry(registryKey)
        ...
        ctx.getSource().sendSuccess(createMessage(
            Component.translatable("commands.vampirelib.tags.tag_key",
            ...
            "commands.vampirelib.tags.element_count",
            "commands.vampirelib.tags.copy_element_names",
            tag.size(),
            page,
    ...
    private static int queryElementTags(final CommandContext<CommandSourceStack> ctx, final int page)
        throws CommandSyntaxException {
        final ResourceKey<? extends Registry<?>> registryKey = getResourceKey(ctx, "registry", ROOT_REGISTRY_KEY)
            .orElseThrow(); // Expect to be always retrieve a resource key for the root registry (registry key)
        final Registry<?> registry = ctx.getSource().getServer().registryAccess().registry(registryKey)
        ...
        ctx.getSource().sendSuccess(createMessage(
            Component.translatable("commands.vampirelib.tags.element",
            ...
            "commands.vampirelib.tags.containing_tag_count",
            "commands.vampirelib.tags.copy_tag_names",
            containingTagsCount,
            page,
            ChatFormatting.DARK_GREEN,
    ...
    private static MutableComponent createMessage(final MutableComponent header,
        ...
        if (count > 0) // Highlight the count text, make it clickable, and append page counters
        {
            ...
            containsComponent = Component.translatable("commands.vampirelib.tags.page_info",
